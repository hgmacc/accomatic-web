...
def crossplot():
    for site in odf.index.get_level_values("sitename").unique():
        ...
        df = pd.concat(
            [odf_site.soil_temperature, mdf_site.era5, mdf_site.merr, mdf_site.jra5],
            axis=1,
        )
        df = df.dropna()
        ...
        ax1.set(xlabel="Observations", ylabel="ERA5")
        ax2.set(xlabel="Observations", ylabel="MERRA-2")
        ax2.set(xlabel="Observations", ylabel="JRA-55")
