...
def get_nc_point_data(pth, site_dict):
    ...
    if os.path.basename(pth).split('_')[-1] == "scaled.nc": var = 'PREC_sur'
...
def read_2D_merra(data_directory, sites):
    f = xr.open_mfdataset(data_directory + '2d_merra2_downloaded.nc', engine="netcdf4")
    mdf = f.to_dataframe()
...
def read_2D_era5(data_directory, sites):
    f = xr.open_mfdataset(data_directory + '2d_era5_downloaded.nc', engine="netcdf4")
...
def read_2D_jra55(data_directory, sites):
    f = xr.open_mfdataset(data_directory + '2d_jra55_downloaded.nc', engine="netcdf4")
...
def build_globsim_csvs(in_dir, out_dir, sites):
    """
    in_dir should hold all scaled, interpolated and downloaded.nc files
    prefix 1D (interp and scaled) files with a wee *ts_*
    
    sites = get_sites('/home/hma000/storage/merra_precip_test/ykl_csvs/sites.csv')
    in_dir = '/home/hma000/storage/merra_precip_test/ykl_ncs/'
    out_dir = '/home/hma000/storage/merra_precip_test/KDI_test/csvs/'
    build_globsim_csvs(in_dir, out_dir, sites)
    """
    # For 1D timeseries
    site_dict = dict(zip(sites.station_number, sites.station_name))
    ts_ncdata = glob.glob(f'{in_dir}/ts*.nc')
    for pth in ts_ncdata:
    ...
    # For 2D timeseries
    df = read_2D_merra(in_dir, sites)
    df.to_csv(f'{out_dir}merra2_downloaded.csv')
    
    df = read_2D_era5(in_dir, sites)
    df.to_csv(f'{out_dir}era5_downloaded.csv')

    df = read_2D_jra55(in_dir, sites)
    df.to_csv(f'{out_dir}jra55_downloaded.csv')
