...
def get_bs_data(n, sample):
    ...
    bs_data = {'means': pd.DataFrame.from_dict(bs_mean), 'se': pd.DataFrame.from_dict(bs_se)}
    return bs_data
...
def bs_boxplot(bs_data, sample):
    ...
    t_star = pd.concat([t_star[col].sort_values(ascending=False).reset_index(drop=True) for col in t_star], axis=1, ignore_index=True)
...
def bs_histogram(n, sample, bs_data):
    ...
    # REGULAR BOOTSTRAP
    bs_var = pd.concat([bs_var[col].sort_values(ascending=False).reset_index(drop=True) for col in x_star], axis=1, ignore_index=True)
    ...
    # PLOTTING
    fig_hist, (ax_t, ax_x) = plt.subplots(nrows=2, ncols=1, sharex=True, figsize=(6, 10))    
    for i, mod in zip(range(len(bs_var.columns)), bs_var.columns):
        ...
        ax_t.hist(ci[mod], histtype='step', linestyle='dashed', color=get_colour(mod), label=f'{mod}_t')
...
def bs_heatmap(ranks):
    ...
    im, cbar = heatmap(data, y_mod, x_ranks, ax=ax,
                    cmap="YlOrRd", cbarlabel="Proportion of instances occupying rank.")
...
def bias_heatmap(x_star):
    ...
    im, cbar = heatmap(data, y_mod, x_ranks, ax=ax,
                    cmap="RdBu", cbarlabel="Proportion of instances occupying +/- bias.")
    texts = annotate_heatmap(im, data=data, valfmt="{x:.2f}")
...
if __name__ == "__main__":
    ...
    bs_data = get_bs_data(n = n,
                           sample = sample[sample.stat == stat].drop(columns=['stat']).reset_index(drop=True))
    rank_dist = get_rank_distribution(x_star = bs_data['means'],
                                      stat=stat)
    
    bs_boxplot(bs_data = bs_data,
               sample = sample[sample.stat == stat].drop(columns=['stat']))
    bs_histogram(n = n,
              sample = sample[sample.stat == stat].drop(columns=['stat']),
              bs_data = bs_data)
    if stat == 'BIAS': bias_heatmap(x_star = bs_data["means"])
    else: bs_heatmap(rank_dist)
